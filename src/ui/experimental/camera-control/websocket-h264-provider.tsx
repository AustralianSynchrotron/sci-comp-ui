...
export const WebsocketH264Provider: React.FC<WebsocketH264ProviderProps> = ({
  ...
  const [currentCropHeight, setCurrentCropHeight] = useState<number>(
    DEFAULT_RESOLUTION.height
  );
  const [currentCropWidth, setCurrentCropWidth] = useState<number>(
    DEFAULT_RESOLUTION.width
  );
  ...
  const [plotData, setPlotData] = useState<PlotDataset>({
    x: [],
    y: [],
    e: [],
  });
  ...
  const decodeSeiMetadata = (nal: Uint8Array) => {
    try {
      ...
      if (uuidMatches(uuid, TIMESTAMP_UUID)) {
        ...
        if (
          !metadataString.includes("x264") &&
          !metadataString.includes("x265")
        ) {
