...
export const CameraControl: React.FC<CameraControlProps> = ({
  ...
  const {
    ...
    clearZoom,
    plotData,
  }: ImageSource = useContext(ImageContext);
  ...
  return (
    <div
      className={cn("relative inline-block h-full w-full space-y-4", className)}
    >
      <canvas
        ref={canvasRef}
        ...
        onKeyUp={handleKeyUp}
        tabIndex={0}
        className={cn("m-0 block h-full w-full", canvasClassName)}
        style={{
      ...
      {popupPos && (
        <div
          className={cn(
            "pointer-events-none fixed z-9999 rounded-sm bg-black/80 px-2 py-1 text-xs text-white",
            popupClassName
          )}
          style={{
            left: popupPos.x + 10,
