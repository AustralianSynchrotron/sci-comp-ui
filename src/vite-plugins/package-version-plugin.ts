...
export function packageVersionPlugin(
  options: PackageVersionPluginOptions = {}
): Plugin {
  const {
    ...
    configResolved(config) {
      try {
        ...
        const packageJson = JSON.parse(
          readFileSync(packageJsonFullPath, "utf-8")
        );
        packageVersion = includeVPrefix
          ? `v${packageJson.version}`
          : packageJson.version;
      } catch (error) {
        console.warn(
          `Failed to read package.json from ${packageJsonPath}:`,
          error
        );
