...
const generatePythonLogData = () => {
  ...
  for (let i = 0; i < 150; i++) {
    ...
    const processedMessage = message
      ...
      .replace(
        "{endpoint}",
        ["/api/users", "/api/data", "/api/auth", "/api/files"][
          Math.floor(Math.random() * 4)
        ]
      )
      .replace(
        "{filename}",
        `document_${Math.random().toString(36).substring(7)}.pdf`
      )
      ...
      .replace(
        "{param}",
        ["limit", "offset", "filter", "sort"][Math.floor(Math.random() * 4)]
      )
      .replace(
        "{field}",
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Log viewer featuring search, pagination, and export
                functionality
              </CardDescription>
            </CardHeader>
            <CardContent>
