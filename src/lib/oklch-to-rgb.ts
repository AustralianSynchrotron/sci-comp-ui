...
export function oklchStringToRgb(oklchString: string): string {
  ...
  if (!match) {
    throw new Error(
      `Invalid OKLCH string format: ${oklchString}. Expected format: oklch(l c h)`
    );
  }
  ...
  const s_ = l - 0.0894841775 * a_comp - 1.291485548 * b_comp;
  ...
  // LMS to XYZ
  const x =
    +1.2268798733 * l_nonlinear -
    0.5578149965 * m_nonlinear +
    0.2813910585 * s_nonlinear;
  const y =
    -0.0405964182 * l_nonlinear +
    1.1122568696 * m_nonlinear -
    0.0716604514 * s_nonlinear;
  const z =
    -0.0763812845 * l_nonlinear -
    0.4214819784 * m_nonlinear +
    1.5861632204 * s_nonlinear;
